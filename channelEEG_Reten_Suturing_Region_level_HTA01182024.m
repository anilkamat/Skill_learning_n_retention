...
addpath(genpath('C:\Users\_Kamat_\Desktop\RPI\ResearchWork\Papers_\Effective_Connectivity\EEG\Phase2_data\LC_EEG_CSD\Retention')) %dataset
load('C:\Users\_Kamat_\Desktop\RPI\ResearchWork\Papers_\Effective_Connectivity\EEG\Phase2_data\HTA\HTA_Suturing_LC_retention.mat'); % new HTA timestamps
...
if isfile('C:\Users\_Kamat_\Desktop\RPI\ResearchWork\Papers_\Effective_Connectivity\Suturing_LC_phase2\EEG_codes_results\Results\Ph2_LC_reten_eeg_sut_GC.mat')
    load('C:\Users\_Kamat_\Desktop\RPI\ResearchWork\Papers_\Effective_Connectivity\Suturing_LC_phase2\EEG_codes_results\Results\Ph2_LC_reten_eeg_sut_GC.mat');
    fg = 1; 
    fprintf('Loading the precomputed analysis, skiping recomputation....');
else
    nsub = 28; %       % total no.of subjects 28
    nT = 3; %3        % number of trials
    freq = [8:0.05:34];           % Frequency range in Alpha or beta range based on analysis
    r = 1;
    fileList = sort(getAllFiles('C:\Users\_Kamat_\Desktop\RPI\ResearchWork\Papers_\Effective_Connectivity\EEG\Phase2_data\LC_EEG_CSD\Retention'));
    nF = numel(fileList);
    ...
    for st = 1:1 %nST              % loop over subtasks
        h = 1;  % counter of subjects each subtask
        for sub_N = 1:nsub %nD              % loop over subjects
            subject = sprintf('P%02dReten',sub_N);
            for f= 1:nF
                fileName = cell2mat(fileList(f));  % name of the file on the folder
                if strcmp(fileName(1:8),subject)== 1 && strcmp(fileName(9:12),'.mat')== 1 && isfield(HTA_Suturing_LC_retention,fileName(1:3))==1
        ...
        %             class = cell2mat(normal_performance(Index,7));
                    ...
                    nT = size(HTA_Suturing_LC_retention.(fileName(1:3)),1);
                    fprintf('%s found, #trials %d \n',fileName, nT);
                    data_Reg = get_LS_channelData(subj_data);  % aggregated regional signals
                    for t = 1:nT                % loop over Trials
                        ST = sprintf('ST%d',st);
                        sst = HTA_Suturing_LC_retention.(fileName(1:3))(t,st);     %start of the subtask
                        est = HTA_Suturing_LC_retention.(fileName(1:3))(t,st+1);     %end of the subtask
                        
%                         sst = HTA_Suturing_LC_retention.(fileName(1:3))(t,phases_idx(st));     %start of the phase
%                         est = HTA_Suturing_LC_retention.(fileName(1:3))(t,phases_idx(st+1));     %end of the phase
                        ...
                        if ~isnan(sst) && ~isnan(est) && Lst >=0
                            ...
                            csvfilename2 = sprintf('C:\\Users\\_Kamat_\\Desktop\\RPI\\ResearchWork\\Papers_\\Effective_Connectivity\\Suturing_LC_phase2\\EEG_codes_results\\Results\\Retention_allChaEEG\\ST%d\\%sTr_%d_alphafreqBP.csv',st,fileName(1:3),t); %sub_N,
                            Y_wind_allChan_alpha = bandpass(Y_subtask_allCh',[8 12],fs)';  % alpha band pass
                            csvwrite(csvfilename2,Y_wind_allChan_alpha)

                            %Write the eeg signals to csv file for
                            %non-linear granger causality analysis.
%                             csvfilename2 = sprintf('C:\\Users\\_Kamat_\\Desktop\\RPI\\ResearchWork\\Papers_\\Effective_Connectivity\\Suturing_LC_phase2\\EEG_codes_results\\Results\\Retention_allChaEEG_Phase\\P%d\\%sTr_%d_alphafreqBP.csv',st,fileName(1:3),t); %sub_N,
%                             Y_wind_allChan_alpha = bandpass(Y_subtask_allCh',[8 12],fs)';  % alpha band pass
%                             csvwrite(csvfilename2,Y_wind_allChan_alpha)
...
%                             sz = numel(GC_temp);
%                             Suturing_GC_reten.(ST).sub(h,:)  = [reshape(GC_temp,1,sz)];
                        end
                        h = h+1;  
                    end
                end
...
if fg == 0
    for j =1:size(fieldnames(Suturing_GC_reten))
        ST_temp = sprintf('ST%d',j);        % Subtask          
        temp = all(Suturing_GC_reten.(ST_temp).sub,2);
        zero_rows = find(temp ==0);
        Suturing_GC_reten.(ST_temp).sub(zero_rows, :) = [];
        Suturing_GC_reten.(ST_temp).sub(:,[1 7 13 19 25],:) = [];       % Remove all the diagonals
    end
    save('C:\Users\_Kamat_\Desktop\RPI\ResearchWork\Papers_\Effective_Connectivity\Suturing_LC_phase2\EEG_codes_results\Results\Ph2_LC_reten_eeg_sut_GC.mat','Suturing_GC_reten')  % saves the connectivity to the local current directory if it doesn't exist
end
%%  ############ Box plot #############
fpath = 'C:\Users\_Kamat_\Desktop\RPI\ResearchWork\Papers_\Effective_Connectivity\Suturing_LC_phase2\EEG_codes_results\Results\Box_Plots_GC_retention';
% plot for the subjects
for j =1:size(fieldnames(Suturing_GC_reten))
    ST = sprintf('ST%d',j);
    if isfield(Suturing_GC_reten.(ST),'sub') && size(Suturing_GC_reten.(ST).sub,1)>1 % check if the file exists and is greater than 1 subject
        close all;
        f = figure('visible','off');
        size(Suturing_GC_reten.(ST).sub(:,1:end-1))
        fprintf('%s %s',subject,ST)
        boxplot(Suturing_GC_reten.(ST).sub(:,1:end),'Labels',connection_GC, 'Colors','b','jitter',0, 'symbol', ''); % 'whisker', inf,  'PlotStyle','Compact'
        set(gca,'FontSize',10,'XTickLabelRotation',45)
        xlabel('Connection')
        ylabel('Granger Causality')
        ylim([-0.01 0.15])
        ttl = sprintf('Suturing LC-Retention: Subtask-%d',j);
        title(ttl);
        baseFileName = sprintf('SuturingLC_Retention_ST%d.png',j);
...
for st =1:13  % loop ove subtasks  
    ST = sprintf('ST%d',st);
    fileName_ = sprintf('C:\\Users\\_Kamat_\\Desktop\\RPI\\ResearchWork\\Papers_\\Effective_Connectivity\\Suturing_LC_phase2\\EEG_codes_results\\Results\\Connectivities\\retention_suturingEEG_GC_ST%d.csv',st);
    if isfield(Suturing_GC_reten,ST)
        T = array2table(Suturing_GC_reten.(ST).sub);
